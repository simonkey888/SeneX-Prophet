...
class WorkerCase(unittest.TestCase):
    ...
    def run(self,job): return run_job(job,self.state,self.target,SOURCE_SHA,now=datetime(2026,8,18,tzinfo=timezone.utc))
    ...
    def test_completion_contract_and_zero_side_effects(self):
        out=self.run(make_job()); required={"job_id","lease_id","attempt","worker_id","worker_version","source_sha","status","task_result","artifacts","tests","acceptance","provenance","side_effects","started_at","finished_at","fixed_job_scope_hash"}
    ...
    def test_scope_hash_tamper_fails_closed(self):
        ...
        with self.assertRaises(JobRejected): self.run(job)
    ...
    def test_crash_after_ack_recovers_and_duplicate_is_idempotent(self):
        ...
        with self.assertRaises(CrashInjected): self.run(job)
        files=list(self.state.glob("*.json")); self.assertEqual(len(files),1); self.assertEqual(json.loads(files[0].read_text())["status"],"ACKED")
        out1=self.run(job); out2=self.run(job); self.assertEqual(out1,out2); self.assertEqual(out1["status"],"SUCCEEDED")
    ...
    def test_crash_during_work_recovers_without_orphan(self):
        ...
        with self.assertRaises(CrashInjected): self.run(job)
        out=self.run(job); self.assertEqual(out["status"],"SUCCEEDED"); self.assertEqual(out["side_effects"]["child_processes"],0)
    def test_cancellation_is_terminal_without_target_effect(self):
        job=make_job(cancelled=True); out=self.run(job); self.assertEqual(out["status"],"CANCELLED"); self.assertEqual(out["artifacts"],[])
    def test_symlink_and_hardlink_escape_fail_closed(self):
        ...
        with self.assertRaises(JobRejected): self.run(make_job())
    ...
    def test_untrusted_python_is_parsed_not_executed_and_cannot_steal_env(self):
        ...
        out=self.run(job); self.assertFalse(marker.exists()); self.assertEqual(out["task_result"]["target_execution"],0); self.assertNotIn(secret,(self.target/"artifacts/source.py").read_text())
    def test_deterministic_replay_and_causal_cutoff(self):
        ...
        a=self.run(job); self.assertEqual([r["id"] for r in a["task_result"]["rows"]],["a","b"])
        ...
        self.assertEqual(self.run(job)["task_result"]["status"],"FAIL")
    def test_statistics_and_robustness(self):
        ...
        self.assertIn(self.run(job)["task_result"]["orientation"],{"IMPROVEMENT","DEGRADATION","INCONCLUSIVE"})
        ...
        out=self.run(job)["task_result"]; self.assertEqual(out["status"],"PASS"); self.assertIn("drop_10pct",out["stresses"])
    def test_shadow_tableau_repair_end_to_end(self):
        ...
        out=self.run(job); self.assertEqual(out["status"],"SUCCEEDED"); self.assertTrue(all(x["pass"] for x in out["task_result"]["checks"])); self.assertTrue((self.target/"artifacts/tableau_shadow_fixed.py").exists())

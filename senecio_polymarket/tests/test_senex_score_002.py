...
def run_reconcile(rows, patch_status=200, patch_body=None):
    ...
    try:
        with patch.object(reconciler.httpx, "AsyncClient", return_value=client), patch.object(reconciler.ccxt, "okx", return_value=FakeExchange()), patch.object(reconciler, "_price_at", side_effect=lambda *args: 101.0 if args[-1] == 900 else 102.0):
...
class SettlementReconcilerTests(unittest.TestCase):
    def test_null_row_is_never_touched(self):
        ...
        self.assertEqual(result["repaired"], 0); self.assertEqual(result["skipped"], 1); self.assertEqual(len(client.patch_calls), 0)
    ...
    def test_conflict_does_not_overwrite_primary_outcome(self):
        ...
        try:
            ...
            reconciler.SUPABASE_URL, reconciler.SUPABASE_KEY, reconciler.BATCH_LIMIT = old
        self.assertEqual(result["repaired"], 0); self.assertEqual(result["conflicts"], 1)
    ...
    def test_patch_failure_is_not_repaired(self):
        ...
        self.assertEqual((result["repaired"], result["errors"]), (0, 1)); self.assertEqual(len(client.patch_calls), 1)
    ...
    def test_multi_batch_repairs_all_eligible_rows(self):
        ...
        class PagedClient(FakeClient):
            ...
            async def get(self, url, params=None):
                self.get_calls.append((url, dict(params or {}))); page = self.pages[self.page_index] if self.page_index < len(self.pages) else []; self.page_index += 1; return FakeResponse(200, list(page))
        client = PagedClient(pages); old = reconciler.SUPABASE_URL, reconciler.SUPABASE_KEY, reconciler.BATCH_LIMIT
        reconciler.SUPABASE_URL, reconciler.SUPABASE_KEY, reconciler.BATCH_LIMIT = "https://example.invalid", "test-key", 50
        try:
        ...
        self.assertEqual(result["repaired"], 51); self.assertEqual(len(client.get_calls), 3)
...
class ProofQualificationTests(unittest.TestCase):
    ...
    def test_scorer_counts_only_proof_qualified_rows(self):
        ...
        score = score_qualified_rows([q, raw, dual]); self.assertEqual((score["verified"], score["wins"], score["losses"], score["win_rate_pct"]), (1, 1, 0, 100.0)); self.assertEqual(len(filter_proof_qualified([q, raw, dual])), 1)

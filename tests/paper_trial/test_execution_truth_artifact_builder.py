...
REQUIRED={"execution_truth_manifest.json","source_time_audit.json","fee_model_conformance.json","sequential_leg_scenarios.json","observed_runner_integration_report.json","settlement_replay_report.json","paper_risk_authority_map.json","monitoring_truth_report.json","regression_summary.json","SHA256SUMS"}
...
def test_builder_produces_hashed_truth_bundle(tmp_path: Path):
    ...
    assert seq["evidence_class"]=="FIXTURE_CONTRACT_EVIDENCE"
    runner=json.loads((tmp_path/"observed_runner_integration_report.json").read_text())
    assert runner["result"]=="PASS"
    assert all(runner["checks"].values())
    assert fee["category_rate_claimed"] is False
    assert all(item["evidence_class"]=="FORMULA_ONLY_SYNTHETIC_VECTOR" for item in fee["vectors"])
    ...
    assert "github_zip UNAVAILABLE_AT_BUILD_TIME" in page

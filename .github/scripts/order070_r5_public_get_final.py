...
def get(url,headers=None,timeout=45):
 req=urllib.request.Request(url,headers=headers or {'Accept':'application/json','Cache-Control':'no-cache','User-Agent':'senex-r5-public-get/1'},method='GET')
 try:
...
def nf(path,query=None):
 u=API+path+(('?'+urllib.parse.urlencode(query)) if query else ''); r=get(u,{'Authorization':f'Bearer {TOKEN}','Accept':'application/json','User-Agent':'senex-r5-public-get-nf/1'})
...
def deploy():
 ...
 if cp.returncode: raise RuntimeError(f'CF_DEPLOY:{cp.returncode}:{dig}')
 urls=re.findall(r'https://[A-Za-z0-9._-]+\.workers\.dev',raw)
 if not urls: raise RuntimeError(f'CF_URL:{dig}')
...
# repository-scoped auth and deploy readback
...
# fresh exact Worker first
EDGE,cfsha=deploy(); boot=None
for i in range(1,41):
 boot=pub(EDGE,'/healthz')
 if boot['http']==200 and boot['headers'].get('x-senex-edge-decision')=='ALLOW_GET_PROXY': break
 time.sleep(2)
if not boot or boot['http']!=200 or boot['headers'].get('x-senex-edge-decision')!='ALLOW_GET_PROXY': raise RuntimeError(f'EDGE_BOOT:{boot}')
write('CLOUDFLARE_METHOD_BOUNDARY.json',{**METHOD_PROOF,'sealed_at':now()})
write('CLOUDFLARE_FINAL_TARGET.json',{'observed_at':now(),'head':TS,'tree':TT,'temporary_worker_url':EDGE,'deploy_output_sha256':cfsha,'credentials_used':False,'get_health':{'http':200,'decision':'ALLOW_GET_PROXY'}})
# re-prime AFTER deploy, bounded, and begin reconciliation immediately
attempts=[]; prime=health=ready=prov=None
for i in range(1,31):
 ...
 attempts.append({'attempt':i,'snapshot':prime['http'],'health':health['http'],'ready':ready['http'],'provenance':prov['http']})
 if all(x['http']==200 for x in [prime,health,ready,prov]): break
 time.sleep(2)
else: write('ORIGIN_GATE.json',{'observed_at':now(),'attempts':attempts}); raise RuntimeError('ORIGIN_GATE_FAILED')
...
write('ORIGIN_LIVE.json',{'observed_at':now(),'attempts':attempts,'health_http':200,'ready_http':200,'provenance':pv,'snapshot_id':prime['body'].get('snapshot_id'),'generation':prime['body'].get('generation'),'canonical_sha256':prime['body'].get('canonical_sha256'),'evidence_status':'EXACT_HEAD_BOUND'})
# concurrent rounds
...
def job(s,base,k,p): return s,k,pub(base,p)
rounds=[]
for n in range(1,9):
 with concurrent.futures.ThreadPoolExecutor(max_workers=10) as ex:
  rows=[f.result() for f in [ex.submit(job,s,base,k,p) for s,base in [('origin',ORIGIN),('edge',EDGE)] for k,p in aps.items()]]
 got={(s,k):r for s,k,r in rows}; statuses={f'{s}:{k}':got[(s,k)]['http'] for s in ('origin','edge') for k in aps}
 row={'round':n,'statuses':statuses}; write(f'ROUND_{n:02d}.json',row)
...
required=['REMOTE_TRUTH.json','AUTH_BUILD_DEPLOY.json','CLOUDFLARE_METHOD_BOUNDARY.json','CLOUDFLARE_FINAL_TARGET.json','ORIGIN_LIVE.json','CONCURRENT_RECONCILIATION.json','LIVE_E2E.json','FINAL_GATE_SUMMARY.json']

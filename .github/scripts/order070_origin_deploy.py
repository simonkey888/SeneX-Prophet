...
TOKEN=os.environ['NORTHFLANK_API_TOKEN']; H={'Authorization':f'Bearer {TOKEN}','Accept':'application/json','Content-Type':'application/json','User-Agent':'senex-order070-deploy/7'}; OUT=Path('order070-origin-evidence'); OUT.mkdir(exist_ok=True)
...
def public(path):
 req=urllib.request.Request(ORIGIN+path,headers={'Accept':'application/json','Cache-Control':'no-cache','User-Agent':'senex-order070-live/7'},method='GET')
 try:
...
if ii.get('deployedSHA')!=TS:
 call('POST',f'/projects/{P}/services/{S}/deployment',{'internal':{'id':ii.get('id') or S,'branch':TB,'buildId':BID}},label='deploy_exact_build_id')
 rec['deployment_request']='POST_BUILD_ID_ONLY'; save()
